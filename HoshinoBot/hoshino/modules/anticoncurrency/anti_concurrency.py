...
HOSHINO_TRIGGER_DICTS = [trigger.prefix.trie,
                         trigger.suffix.trie, trigger.keyword.allkw, trigger.rex.allrex]
        ...
        for trigger_word in trigger_group:
            for trigger_words_dict in HOSHINO_TRIGGER_DICTS:
                if trigger_word in trigger_words_dict:
                    ...
                    sf.func = sf_wrapper(
                        sf.func, trigger_group_index, trigger_word)
                    sv.logger.info(
                        f'Succeed to add wrapper on trigger `{trigger_word}`')
                    break
