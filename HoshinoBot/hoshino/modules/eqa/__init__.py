...
admins = hoshino.config.SUPERUSERS[0]  # 默认改为superuser的第一位
admins = set((admins if isinstance(admins, list)
              else [admins]) + _bot.config.SUPERUSERS)
...
async def answer(ctx):
    ...
    if priority_self_answer:
        self_list = util.filter_list(
            ans_list, lambda x: user_id == x['user_id'])
...
async def show_question(ctx, target, show_all=False):
    ...
    if not show_all:
        ...
        # 如果跟了@人的对象
        target = list(int(i) for i in re.findall(
            r'\[CQ:at,qq=(\d+)]', target.strip()))
        is_at = bool(target)
    ...
    for qq in target:
        ...
        if not show_all:  # 不是显示全部的话就筛选个人
            ...
            all_list = util.filter_list(
                ans_list, lambda x: True in list(not i['is_me'] for i in x))
            # 这人个人问答
            priority_list = util.filter_list(
                ans_list, lambda x: True in list(i['is_me'] for i in x))

            ans_list = sum(list(util.get_all_ans_list_by_qq(q, db_list)
                                for q in admins), all_list)
        ...
        msg = "{}{}{}{}\n".format(
            msg, head, msg_context if ans_list else f'还没有设置过问题呢', priority_msg)
    return msg
...
async def del_question(ctx, target, clear=False):
    ...
    is_super_admin = ctx['user_id'] in admins
    is_group_admin = util.is_group_admin(
        ctx) if config['rule']['only_admin_can_delete'] else True
