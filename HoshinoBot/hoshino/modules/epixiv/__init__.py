...
try:
    epixiv.login(hoshino.config.Pixiv_UserName, hoshino.config.Pixiv_PassWord)
    pass
...
async def print_illusts(ctx, illusts):
    for illust in illusts:
        ...
        original = short_url.short(
            image_urls.original) if config.setting.short_url_enable else image_urls.original
        msg = {
            'title': illust.title,
            'id': illust.id,
...
async def show_meta_pages(ctx, keyword: str):
    ...
    for index, value in enumerate(images):
        original = short_url.short(
            value) if config.setting.short_url_enable else value
        msg = {
            'original': original,
